...
impl<'dom> LayoutDom<'dom, Element> {
    ...
    /// Iterate complete immutable attribute facts while layout owns DOM access. This keeps
    /// bounded selector accounting single-pass without constructing Script-thread DOM roots.
    #[expect(unsafe_code)]
    #[inline]
    pub(crate) fn each_attr_for_layout<F>(self, mut callback: F)
    where
        F: FnMut(&Namespace, &LocalName, &AttrValue),
    {
        let storage = unsafe { self.unsafe_get().attrs.borrow_for_layout() };
        for entry in storage.iter() {
            callback(
                entry.namespace_for_layout(),
                entry.local_name_for_layout(),
                entry.value_for_layout(),
            );
        }
    }
