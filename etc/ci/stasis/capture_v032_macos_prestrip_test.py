...
class CaptureV032MacosPrestripTests(unittest.TestCase):
    ...
    def make_verify_fixture(self, root: Path) -> tuple[SimpleNamespace, bytes]:
        ...
        return args, poststrip

    def test_verify_binds_one_hashed_target_to_cargo_and_immutable_release(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            args, poststrip = self.make_verify_fixture(Path(directory).resolve())
            with (
                mock.patch.object(capture, "RELEASE_BINARY_SHA256", capture.sha256(args.release_binary)),
                mock.patch.object(capture, "RELEASE_BINARY_BYTES", len(poststrip)),
            ):
                capture.verify(args)
            ...
            self.assertTrue(result["poststripHashedTargetMatchesImmutableRelease"])

    def test_verify_rejects_a_second_hashed_stasis_target(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory).resolve()
            args, poststrip = self.make_verify_fixture(root)
            (root / "target" / "production-stripped" / "deps" / "stasis-fedcba9876543210").write_bytes(poststrip)
            with (
                mock.patch.object(capture, "RELEASE_BINARY_SHA256", capture.sha256(args.release_binary)),
                mock.patch.object(capture, "RELEASE_BINARY_BYTES", len(poststrip)),
