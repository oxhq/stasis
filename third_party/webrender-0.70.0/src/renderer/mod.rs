...
/// Physical ownership of WebRender's backend and scene threads.
#[doc(hidden)]
pub struct BackendThreadHandles {
    handles: Vec<(&'static str, JoinHandle<()>)>,
}

impl BackendThreadHandles {
    #[doc(hidden)]
    pub fn new(
        render_backend: JoinHandle<()>,
        scene_builder: JoinHandle<()>,
        low_priority_scene_builder: Option<JoinHandle<()>>,
    ) -> Self {
        let mut handles = vec![
            ("render backend", render_backend),
            ("scene builder", scene_builder),
        ];
        if let Some(handle) = low_priority_scene_builder {
            handles.push(("low-priority scene builder", handle));
        }
        Self { handles }
    }
    ...
    pub fn join_all(&mut self) -> thread::Result<()> {
        ...
        for (label, handle) in mem::take(&mut self.handles) {
            if let Err(payload) = handle.join() {
                error!("WebRender {} thread panicked during shutdown", label);
...
pub struct Renderer {
    ...
    backend_threads: BackendThreadHandles,
    pub device: Device,
...
impl Renderer {
    ...
    /// Wait until every WebRender-owned backend and scene thread has physically exited.
    ///
