...
use crate::renderer::{
    ...
    BackendThreadHandles, RendererError, ShaderPrecacheFlags, VERTEX_DATA_TEXTURE_COUNT,
...
pub fn create_webrender_instance(
    ...
    let scene_builder_thread = thread::Builder::new().name(scene_thread_name.clone()).spawn(move || {
    ...
    let (low_priority_scene_tx, low_priority_scene_builder_thread) = if options.support_low_priority_transactions {
        ...
        let low_priority_scene_builder_thread = thread::Builder::new().name(lp_scene_thread_name.clone()).spawn(move || {
        ...
        (low_priority_scene_tx, Some(low_priority_scene_builder_thread))
    } else {
        (scene_tx.clone(), None)
    };
    ...
    let render_backend_thread = thread::Builder::new().name(rb_thread_name.clone()).spawn(move || {
    ...
    let mut renderer = Renderer {
        result_rx,
        api_tx: api_tx.clone(),
        backend_threads: BackendThreadHandles::new(
            render_backend_thread,
            scene_builder_thread,
            low_priority_scene_builder_thread,
        ),
        device,
